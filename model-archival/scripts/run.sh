...
#
#   # Full run with serial queue (default: daytime cap 0.75 MB/s 07:00–23:00, unlimited overnight):
#   bash run.sh --all --queue-mode serial --rehash
#
...
PLAN_RC=0
PLAN_OUT=$(cd "$REPO_DIR" && uv run archiver --registry "$REGISTRY_FILE" download "${DOWNLOAD_ARGS[@]}" --dry-run 2>&1) || PLAN_RC=$?
echo "$PLAN_OUT"
...
" 2>/dev/null || echo "/tmp/archiver/logs")
    ...
    DL_RC=0
    cd "$REPO_DIR" && uv run archiver --registry "$REGISTRY_FILE" download "${DOWNLOAD_ARGS[@]}" &
    _ARCHIVER_PID=$!
...
if $DRY_RUN; then
    ...
    STATUS_OUT=$(cd "$REPO_DIR" && uv run archiver --registry "$REGISTRY_FILE" status 2>&1 || true)
    echo "$STATUS_OUT"
