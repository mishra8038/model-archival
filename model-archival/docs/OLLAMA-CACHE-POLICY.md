...
1. **Free space on supermicro** by deleting Ollama blobs *only after* the same model is **fully synced** to the archival VM (`scripts/ollama-sync.sh` → `x@192.168.8.65`, destination **rotates** across `d5` → `d2` → `d3` → `d1` unless you set a fixed `ARCHIVAL_VM_DEST`).
