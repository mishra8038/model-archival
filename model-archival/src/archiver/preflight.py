...
def check_registry(registry: Registry) -> None:
    ...
    for m in registry.models:
        ...
        # 0 = optional “queue first” band (lower than priority 1); see registry-specialists notes.
        if m.priority not in (0, 1, 2, 3, 4):
            errors.append(f"{m.id}: invalid priority {m.priority} (must be 0–4)")
