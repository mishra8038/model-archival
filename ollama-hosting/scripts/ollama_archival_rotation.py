...
def cmd_print_archive_roots(cycle_raw: str | None) -> None:
    for _, pth in _parse_cycle(cycle_raw or os.environ.get("ARCHIVAL_VM_SITE_CYCLE")):
        print(pth)
...
def main() -> None:
    ...
    p_roots = sub.add_parser(
        "print-archive-roots",
        help="Print one archival Ollama root path per line (same cycle as sync / inventory)",
    )
    ...
    elif args.cmd == "print-archive-roots":
        cmd_print_archive_roots(args.cycle)
    else:
        cmd_advance(
            args.repo,
            args.used_dest,
