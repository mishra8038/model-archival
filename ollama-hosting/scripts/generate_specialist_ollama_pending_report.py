...
# This script lives in ollama-hosting/scripts/; HF registry YAML stays in model-archival/config/.
_SCRIPT = Path(__file__).resolve()
_HOST_ROOT = _SCRIPT.parent.parent
_REPO_ROOT = _HOST_ROOT.parent
_MA_ROOT = _REPO_ROOT / "model-archival"

SPEC_PATH = _MA_ROOT / "config/registry-specialists.yaml"
MAIN_PATH = _MA_ROOT / "config/registry.yaml"
FAILED_PATH = _MA_ROOT / "config/failed-models-registry.yaml"
OUT_PATH = _HOST_ROOT / "docs/SPECIALIST-HF-PENDING-OLLAMA.md"
CACHE_PATH = _HOST_ROOT / "docs/data/ollama-registry-size-cache.json"
...
def main() -> None:
    ...
    lines.extend(
        [
            ...
            "`model-archival/config/failed-models-registry.yaml`. Refresh that file with **`uv run archiver failed-registry`** "
    ...
    OUT_PATH.write_text("\n".join(lines) + "\n")
    print(
        f"Wrote {OUT_PATH.relative_to(_HOST_ROOT)} ({n_union} rows; {n_spec} specialist, {n_failed_unique} distinct failed ids)"
    )
